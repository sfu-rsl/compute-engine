...
namespace compute
{
    ...
    public:
        ...
        virtual void setup(MatPtr<DataType> A, BufferPtr<DataType> x, BufferPtr<DataType> b) {
            (void)(A);
            (void)(x);
            (void)(b);
        };
        virtual bool solve(MatPtr<DataType> A, BufferPtr<DataType> x, BufferPtr<DataType> b) = 0;
    ...
    public:
        ...
        virtual void setup(MatPtr<DataType> Hpp, MatPtr<DataType> Hpl, MatPtr<DataType> Hllinv, BufferPtr<DataType> x, BufferPtr<DataType> b) = 0;

        virtual bool solve(MatPtr<DataType> Hpp, MatPtr<DataType> Hpl, MatPtr<DataType> Hllinv, BufferPtr<DataType> x, BufferPtr<DataType> b) = 0;
    };
    ...
    {
    public:
        void analyzePattern_block(MatPtr<typename Method::Scalar> A, const typename Method::MatrixType &matrix, bool LDLT)
        {
            ...
            Eigen::SparseMatrix<double, Eigen::ColMajor> pattern(A->num_scalar_rows(), A->num_scalar_cols());
            pattern.selfadjointView<Eigen::Upper>() = matrix.template selfadjointView<Method::UpLo>().twistedBy(this->m_P);
    ...
    {
    private:
        using decomp_method = Eigen::SimplicialLDLT<Eigen::SparseMatrix<DataType, Eigen::ColMajor>, Eigen::Upper>;
    ...
    public:
        LDLTSolver(bool use_block_ordering = false) : first_iter(true), block_ordering(use_block_ordering) {}
    ...
    {
    private:
        using decomp_method = Eigen::SimplicialLLT<Eigen::SparseMatrix<DataType, Eigen::ColMajor>, Eigen::Upper>;
        Decomp<decomp_method> decomp;
    ...
    public:
        LLTSolver(bool use_block_ordering = false) : first_iter(true), block_ordering(use_block_ordering) {}
